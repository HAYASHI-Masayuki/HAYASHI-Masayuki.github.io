...
* ここまででルーティングの全体像は把握できたかと。では、詳細に入ろう。
* だがその前にもう一つ、ルーティング関連の重要なクラスを、先に理解したい。
* まずRouteファサードクラス。これはrouterサービスへのファサードで、routerサービスはRouting\Routerのインスタンスだ。RoutingServiceProviderで用意される。
* また、Routeファサードでのルートの設定方法は、大きく分けて3つあった。HTTPメソッド名のメソッド、resource(), また、group()経由。
* これら3つをそれぞれ別々に紹介したのにはわけがあって、この3パターンで、処理の内容が違う。処理のためのクラスがまず違う。
* HTTPメソッド名のメソッドから登録する場合、Router::get()等はすべて、Router::addRoute()を呼び出していて、ここでは新たにルート(Routing\Route)を生成した上で、ルートコレクションに登録して、さらに戻り値として戻している。つまり、Route::get()からチェーンして続くメソッドは、すべてRouting\Routeのメソッドとなる。
* またresource()は、今度はPendingResourceRegistrationというクラスを返す。名前通り、これはリソースルートの登録を、保留しておくという特殊なクラスだ。なぜか？　resource()は続いてonly()やexpect()を指定できる。resource()の時点ですべて登録しておいて、その後only()やらを実行したときに、一度登録したものを登録解除するようなことはややこしいのだろう。それだけでなく、resource()に続いてミドルウェアをまとめて設定したりすることもできる。そういう場合に、一度登録したものを毎回
  * あー、というか登録→onlyで削除→ミドルウェア設定→onlyで再追加、とかありうる？
  * で、じゃあどう保留しているのを後でどうにかしてるのか。単純にデストラクタで登録するように設定されている。
